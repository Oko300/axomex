  ...
  try {
    ...
    try {
      ...
      const followRes = await axios.get(
        `https://api.twitter.com/2/users/${user.id}/following?max_results=100`,
        { headers: { Authorization: `Bearer ${accessToken}` } }
      );
    ...
    const userData = JSON.stringify({ username: user.username, isFollowing });
    res.setHeader('Set-Cookie', [
      `x_user=${encodeURIComponent(userData)}; Path=/; SameSite=Lax; Max-Age=3600`,
